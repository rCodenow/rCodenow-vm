...
const {loadCcx} = require('../../../../src/lib/cc-extension-manager')
...
const builtinExtensions = require('rcExts');
...
class ExtensionManager {
    ...
    async loadExtensionURL(extensionURL) {
        ...
        const isCcx = this.securityManager.isCcx(extensionURL);
        if (isCcx) {
            
            const base64Content = extensionURL.replace(/^data:application\/zip;base64,/, '').replace(/^data:application\/x-zip-compressed;base64,/, '');
            const byteCharacters = atob(base64Content);
            
            // Convert byteCharacters to Uint8Array
            ...
            const file = new File([blob], 'ext.cc3');
            loadCcx(file)
            return;
        }
